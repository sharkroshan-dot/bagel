...
class Templates(Static):
    ...
    BINDINGS = [
        Binding(config.CONFIG.hotkeys.new, "new_template", "New"),
        Binding(
            config.CONFIG.hotkeys.home.new_transfer, "new_transfer", "New Transfer Template"
        ),
        Binding(config.CONFIG.hotkeys.edit, "edit_template", "Edit"),
        Binding(config.CONFIG.hotkeys.delete, "delete_template", "Delete"),
    ...
    def _create_templates_widgets(self, container: Container) -> None:
        ...
        for index, template in enumerate(self.templates):
            ...
            if template.isTransfer:
                ...
                widget = Container(
                    Label(
                        f"[{color}]{config.CONFIG.symbols.category_color}[/{color}]",
                        classes="dot",
                    ),
                    Label(f"{template.label}", classes="label"),
                    id=f"template-{template.id}",
