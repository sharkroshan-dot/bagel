...
class RecordTableBuilder:
            ...
            match self.page_parent.filter["offset_type"]:
                ...
                case "month":
                    ...
                    week_start = record.date - timedelta(
                        days=(record.date.weekday() - config.CONFIG.defaults.first_day_of_week)
                        % 7
                    )
    ...
    def _get_flow_icon(self, recordHasSplits: bool, is_income: bool) -> str:
        if recordHasSplits and not self.show_splits:
            ...
            flow_icon_positive = f"[green]{config.CONFIG.symbols.amount_positive}[/green]"
            flow_icon_negative = f"[red]{config.CONFIG.symbols.amount_negative}[/red]"
    ...
    def _format_record_fields(self, record, flow_icon: str) -> tuple[str, str]:
        if record.isTransfer:
            ...
            category_string = f"[{color_tag}]{config.CONFIG.symbols.category_color}[/{color_tag}] {record.category.name}"
    ...
    def _add_split_rows(self, table: DataTable, record, flow_icon: str) -> None:
        ...
        split_flow_icon = (
            f"[red]{config.CONFIG.symbols.amount_negative}[/red]"
            if record.isIncome
            else f"[green]{config.CONFIG.symbols.amount_positive}[/green]"
        )
        line_char = f"[{color}]{config.CONFIG.symbols.line_char}[/{color}]"
        finish_line_char = f"[{color}]{config.CONFIG.symbols.finish_line_char}[/{color}]"
    ...
    def _get_split_status_icon(self, split) -> str:
        if split.isPaid:
            return f"[green]{config.CONFIG.symbols.split_paid}[/green]"
        else:
            return f"[grey]{config.CONFIG.symbols.split_unpaid}[/grey]"
        ...
        for person in persons:
            if person.splits:  # Person has splits for this month
                ...
                for split in person.splits:
                    record = split.record
                    paid_icon = (
                        f"[green]{config.CONFIG.symbols.split_paid}[/green]"
                        if split.isPaid
                        else f"[red]{config.CONFIG.symbols.split_unpaid}[/red]"
                    )
                    date = (
                    ...
                    category = f"[{record.category.color.lower()}]{config.CONFIG.symbols.category_color}[/{record.category.color.lower()}] {record.category.name}"
                    ...
                    amount = (
                        f"[red]{config.CONFIG.symbols.amount_negative}[/red] {split.amount}"
                        if record.isIncome
                        else f"[green]{config.CONFIG.symbols.amount_positive}[/green] {split.amount}"
                    )
                    account = f"â†’ {split.account.name}" if split.account else "-"
