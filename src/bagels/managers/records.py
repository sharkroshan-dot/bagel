...
def get_records(
    ...
    try:
        ...
        query = query.filter(Record.deletedAt.is_(None))
...
def export_records_to_csv(
    ...
    records = get_records(
        ...
        category_piped_names=category_piped_names,
        ...
        writer.writerow(
            {
                ...
                "account": record.account.name if record.account else "",
                "category": record.category.name if record.category else "",
                "isIncome": int(record.isIncome),
                "isTransfer": int(record.isTransfer),
            }
        )
    return output.getvalue()


def export_category_report_to_csv(offset: int = 0, offset_type: str = "month"):
    records = get_records(offset=offset, offset_type=offset_type)
    category_sums: dict[str, float] = {}
    for record in records:
        if record.isTransfer or record.isIncome:
            continue
        category_name = record.category.name if record.category else "Uncategorized"
        category_sums[category_name] = category_sums.get(category_name, 0.0) + record.amount

    output = StringIO()
    writer = csv.DictWriter(output, fieldnames=["category", "expense"])
    writer.writeheader()
    for category, expense in sorted(category_sums.items(), key=lambda item: item[1], reverse=True):
        writer.writerow({"category": category, "expense": expense})
    return output.getvalue()


def import_records_from_csv(csv_text: str, field_mapping: dict[str, str] = None):
    ...
    for row in reader:
        data = {
            "date": row.get(field_mapping.get("date", "date"), ""),
            "label": row.get(field_mapping.get("label", "label"), ""),
            "amount": row.get(field_mapping.get("amount", "amount"), ""),
            "account": row.get(field_mapping.get("account", "account"), ""),
            "category": row.get(field_mapping.get("category", "category"), ""),
            "isIncome": row.get(field_mapping.get("isIncome", "isIncome"), "0"),
        }
        records.append(data)
    return records
...
def get_deleted_records():
    ...
    try:
        ...
        query = query.filter(Record.deletedAt.isnot(None))
        return query.order_by(Record.deletedAt.desc()).all()
    finally:
        session.close()
...
def restore_record(record_id: int):
    ...
    try:
        ...
        if record and record.deletedAt is not None:
